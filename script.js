...
(function () {
  ...
  function renderDynamicPaypalAdd(mountEl, buttonKey) {
    if (!mountEl) return;
    const buttonId = hostedButtonId(buttonKey);
    if ((buttonKey === "ehive-one-no-license" || buttonKey === "ehive-one-with-license")
      && isPlaceholderId(buttonId)) {
      // Never display a checkout button until PayPal's stored price is confirmed.
